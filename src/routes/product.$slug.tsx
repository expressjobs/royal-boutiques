...
export const Route = createFileRoute("/product/$slug")({
  head: ({ params, loaderData }) => {
    ...
    const desc =
      p.description ?? "Shop this piece at Royal Boutiques — luxury fashion, timeless elegance.";
    return {
      meta: [
...
function ProductPage() {
    ...
    return (
      <BoutiqueLayout>
        <div className="py-32 text-center">
          ...
          <Link
            to="/shop"
            className="mt-6 inline-block text-sm uppercase tracking-widest border-b border-charcoal"
          >
            Back to shop
          </Link>
        </div>
  ...
  const gallery = [product.image_url, ...images.map((i) => i.image_url)].filter(
    Boolean,
  ) as string[];
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        ...
        <div className="grid lg:grid-cols-2 gap-12">
              ...
              <div className="grid grid-cols-5 gap-2 mt-4">
                {gallery.map((src, i) => (
                  <button
                    ...
                    className={`aspect-square bg-soft rounded-xl overflow-hidden border-2 transition ${i === activeImg ? "border-gold" : "border-transparent hover:border-charcoal/20"}`}
                  >
          ...
          <div className="lg:sticky lg:top-28 self-start">
            {product.category && (
              <p className="eyebrow text-charcoal/50">{product.category.name}</p>
            )}
            ...
            <div className="flex items-center gap-3 mt-4">
              <div className="flex items-center gap-1">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star
                    key={i}
                    className={`h-3.5 w-3.5 ${i < Math.round(Number(product.rating)) ? "fill-gold text-gold" : "text-charcoal/20"}`}
                  />
                ))}
              </div>
            ...
            <div className="flex gap-3 items-baseline mt-6">
              {onSale ? (
                <>
                  ...
                  <span className="text-lg font-serif line-through text-charcoal/40">
                ...
                <div className="flex flex-wrap gap-2">
                  {product.sizes.map((s) => (
                    <button
                      key={s}
                      onClick={() => setSize(s)}
                      className={`min-w-[48px] h-12 px-4 border text-sm transition ${size === s ? "border-charcoal bg-charcoal text-white" : "border-charcoal/20 hover:border-charcoal"}`}
                    >
                      {s}
                    </button>
                  ))}
            ...
            {product.colors.length > 0 && (
              <div className="mt-6">
                <p className="eyebrow mb-3">
                  Color{" "}
                  {color && (
                    <span className="ml-2 normal-case tracking-normal text-charcoal/60">
                      {color}
                    </span>
                  )}
                </p>
                <div className="flex flex-wrap gap-2">
                  {product.colors.map((c) => (
                    <button
                      key={c}
                      onClick={() => setColor(c)}
                      className={`px-4 h-10 border text-xs uppercase tracking-wider transition ${color === c ? "border-charcoal bg-charcoal text-white" : "border-charcoal/20 hover:border-charcoal"}`}
                    >
                      {c}
                    </button>
                  ))}
            ...
            <div className="mt-8 flex items-center gap-4">
              <div className="flex items-center border border-charcoal/20 h-12">
                <button onClick={() => setQty(Math.max(1, qty - 1))} className="px-3 h-full">
                  <Minus className="h-3 w-3" />
                </button>
                <span className="px-4 text-sm">{qty}</span>
                <button onClick={() => setQty(qty + 1)} className="px-3 h-full">
            ...
            <div className="mt-8 grid grid-cols-2 gap-4 text-xs text-charcoal/60">
              <div className="flex items-center gap-2">
                <Truck className="h-4 w-4" /> Free shipping over KES 5,000
              </div>
              <div className="flex items-center gap-2">
...
function ReviewSection({ productId, reviews }: { productId: string; reviews: any[] }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase
      .from("reviews")
      .upsert({ product_id: productId, user_id: user.id, rating, comment });
    setSubmitting(false);
  ...
  return (
    <div className="grid lg:grid-cols-[1fr_360px] gap-12">
      <div className="space-y-6">
        {reviews.length === 0 && (
          <p className="text-charcoal/60 text-sm">No reviews yet. Be the first.</p>
        )}
        {reviews.map((r) => (
          <div key={r.id} className="border-b border-charcoal/10 pb-6">
            <div className="flex items-center gap-2 mb-2">
              {Array.from({ length: 5 }).map((_, i) => (
                <Star
                  key={i}
                  className={`h-3 w-3 ${i < r.rating ? "fill-gold text-gold" : "text-charcoal/20"}`}
                />
              ))}
      ...
      <form onSubmit={submit} className="bg-nude rounded-2xl p-6 h-fit">
        ...
        <div className="flex gap-1 mb-3">
          {[1, 2, 3, 4, 5].map((n) => (
            <button key={n} type="button" onClick={() => setRating(n)}>
              <Star
                className={`h-5 w-5 ${n <= rating ? "fill-gold text-gold" : "text-charcoal/20"}`}
              />
            </button>
          ))}
        </div>
        ...
        <button
          type="submit"
          disabled={submitting}
          className="mt-3 w-full bg-charcoal text-white py-3 text-[11px] uppercase tracking-[0.2em] font-semibold disabled:opacity-50"
        >
