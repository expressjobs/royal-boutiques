...
Move Royal Boutiques to: **GitHub → your Supabase → Cloudflare Workers → royabotiques.com**.
...
| Auth providers: email/password + Google                          | Supabase Auth           | Your Supabase Auth                 |
...
| Domain                                                           | `*.lovable.app`         | `royabotiques.com`                 |
...
- Site URL: `https://royabotiques.com`
- Redirect URLs: `https://royabotiques.com/**`, `http://localhost:5173/**`

> Google OAuth now uses native `supabase.auth.signInWithOAuth({ provider: "google", options: { redirectTo: ... } })` in `src/routes/auth.tsx`. Configure the Supabase Google provider and redirect URLs before production cutover.
...
The current Vite/TanStack build emits:
...
Add `royabotiques.com` to your Cloudflare account, then:

- Dashboard → Workers → `royal-boutiques` → **Triggers → Custom Domains** → add `royabotiques.com` + `www.royabotiques.com`. TLS is automatic.

### M-Pesa callback URL

Update your Daraja app's callback URL to:
`https://royabotiques.com/api/public/mpesa/callback` (route exists at `src/routes/api/public/mpesa.callback.ts`).
...
| File / dep                                                                                                      | What to do                                                                                                                                                                                                                                                                                                                              |
| --------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `@lovable.dev/cloud-auth-js` (package.json)                                                                     | Removed; Google OAuth uses native Supabase OAuth.                                                                                                                                                                                                                                                                                       |
...
| `src/routes/auth.tsx` Google sign-in                                                                            | Uses `supabase.auth.signInWithOAuth({ provider: "google", options: { redirectTo: window.location.origin + "/account" } })`.                                                                                                                                                                                                             |
...
- [x] Lovable broker call in `auth.tsx` swapped for native Supabase OAuth
- [x] `@lovable.dev/cloud-auth-js` uninstalled
...
- [ ] `royabotiques.com` + `www` added as Custom Domains, DNS green
- [ ] Smoke test: browse catalog, sign up, place test M-Pesa order, verify webhook hits, check admin dashboard
- [ ] DNS for the old Lovable preview either retired or 301-redirected to `royabotiques.com`
